...
if(isset($_POST['submit'])){
	if($_POST['password1']==$_POST['password2']){
		...
		if($checknumrows==1){
			...
			$sql = "INSERT INTO users(username, password, email, verifystring, active) VALUES('" . $_POST['username'] . "', '" . $_POST['password1'] . "', '" . $_POST['email'] . "', '" . addslashes($randomstring) . "', 0);";
			mysql_query($sql);
			$email_body=<<<_MAIL_
			Hi $validusername,
...
_MAIL_;
			mail($_POST['email'], " Bidtastic auctions " . " User verification ", $email_body);
			require("header.php");
			echo "A link has been emailed to the address you entered. Please follow the link in the email to validate your account.";
		}
	}
	else{header("Location: " . $config_basedir) . "register.php?error=pass)";
	...
	if(isset($_GET['error'])){
	switch($_GET['error']){
		case "pass": echo "Passwords do not match";
		break;
		case "taken": echo "Username taken, please use another.";
		break;
		case "no": echo "Incorrect login details";
		break;
	}}

?>
<h2>Register</h2>
To register on the Bidtastic auctions, fill in the form below.
