<?php
	...
	if(isset($_POST['submit'])){
		...
		if($validdate == TRUE) { 
			...
			. $_POST['hour'] . ":" . $_POST['minute'] . ":00";

			$itemsql = "INSERT INTO items(user_id, cat_id, name, startingprice, description, dataends) 
			VALUES(" . $_SESSION['USERID'] . ", " . $_POST['cat'] 
...
if(isset($_GET['error'])){
	switch($_GET['error']){
		case "date": echo "<strong>Invalid choice-please choose another!</strong>";
		break;
	}
}
?>
...
<table>
<?php
$catsql="SELECT * FROM categories ORDER BY cat;";
